...
class QarnotBoxSchedAndrei():
    ...
    def scheduleInstances(self, qtask):
        ...
        self.bs.reject_jobs(qtask.waiting_instances.copy())
        #assert False
    ...
    def onJobCompletion(self, job, direct_job = -1):
        ...
        if direct_job != -1:
            ...
            self.logger.info("[{}]--- QBox {} received direcltly {} instances of {} for the priority group {}".format(self.bs.time(), self.name, len(direct_job), direct_job.qtask_id, direct_job.priority_group))
        else:
            qm.pop_job()
